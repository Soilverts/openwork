                        ...
                        <div class="flex items-center gap-3 mb-2">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-gray-12">
                                    {currentQuestion()!.header || t("question_modal.question")}
                                </h3>
                                <div class="text-xs text-gray-11 font-medium">
                                    {t("question_modal.question_of").replace("{current}", String(currentIndex() + 1)).replace("{total}", String(props.questions.length))}
                                </div>
                            </div>
                        </div>
                        ...
                        <Show when={currentQuestion()!.custom}>
                            <div class="mt-4 pt-4 border-t border-dls-border">
                                <label class="block text-xs font-semibold text-dls-secondary mb-2 uppercase tracking-wide">
                                    {t("question_modal.custom_answer")}
                                </label>
                                <input
                                    type="text"
                                    value={customInput()}
                                    ...
                                    placeholder={t("question_modal.type_answer")}
                                    onKeyDown={(e) => {
                                        if (e.key === "Enter") {
                                            if (e.isComposing || e.keyCode === 229) return;
                                            e.stopPropagation();
                        ...
                        <div class="flex gap-2">
                            <Show when={currentQuestion()?.multiple || currentQuestion()?.custom}>
                                <Button onClick={handleNext} disabled={!canProceed() || props.busy} class="!px-6">
                                    {isLastQuestion() ? t("question_modal.submit") : t("question_modal.next")}
