...
export default function IdentitiesView(props: IdentitiesViewProps) {
  ...
  const lastActivityLabel = createMemo(() => {
    ...
    if (minutes < 60) return t("identities.minutes_ago").replace("{n}", String(minutes));
    const hours = Math.floor(minutes / 60);
    if (hours < 24) return t("identities.hours_ago").replace("{n}", String(hours));
    const days = Math.floor(hours / 24);
    return t("identities.days_ago").replace("{n}", String(days));
  });
  ...
  const loadAgentFile = async () => {
    ...
    if (!id) {
      resetAgentState();
      setAgentError(t("identities.worker_scope_unavailable"));
      return;
    }
  ...
  const saveAgentFile = async () => {
    ...
    try {
      ...
      if (error instanceof OpenworkServerError && error.status === 409) {
        setAgentError(t("identities.agent_conflict"));
      } else {
        setAgentError(formatRequestError(error));
      }
    } finally {
  ...
  const sendTestMessage = async () => {
    ...
    try {
      ...
      const base = t("identities.send_dispatched").replace("{sent}", String(result.sent)).replace("{attempted}", String(result.attempted));
  ...
  const refreshAll = async (options?: { force?: boolean }) => {
    ...
    try {
      ...
      if (!id) {
        ...
        setSlackIdentities([]);
        setHealthError(t("identities.worker_scope_unavailable_reconnect"));
        ...
        if (!healthRes.ok) {
          const message =
            (healthRes.json && typeof (healthRes.json as any).message === "string")
              ...
              : t("identities.health_unavailable").replace("{status}", String(healthRes.status));
      ...
      if (isOpenCodeRouterIdentities(tgRes)) {
        ...
        setTelegramIdentities([]);
        setTelegramPairingCode(null);
        setTelegramIdentitiesError(t("identities.telegram_identities_unavailable"));
      }
      ...
      if (isOpenCodeRouterIdentities(slackRes)) {
        ...
        setSlackIdentities([]);
        setSlackIdentitiesError(t("identities.slack_identities_unavailable"));
      }
    ...
    try {
      ...
      if (result.ok) {
        ...
        if (access === "private" && pairingCode) {
          ...
          setTelegramStatus(t("identities.telegram_private_saved").replace("{code}", pairingCode));
        } else {
          setTelegramPairingCode(null);
        }
        ...
        if (username) {
          ...
          if (access !== "private" || !pairingCode) {
            setTelegramStatus(t("identities.telegram_saved_username").replace("{username}", normalized || String(username)));
          }
        } else {
          if (access !== "private" || !pairingCode) {
            setTelegramStatus(result.applied === false ? t("identities.saved_pending") : t("identities.saved"));
          }
        }
      } else {
        setTelegramError(t("identities.failed_to_save"));
      }
  ...
  const deleteTelegram = async (identityId: string) => {
    ...
    try {
      ...
      if (result.ok) {
        setTelegramBotUsername(null);
        setTelegramPairingCode(null);
        setTelegramStatus(result.applied === false ? t("identities.deleted_pending") : t("identities.deleted"));
      } else {
        setTelegramError(t("identities.failed_to_delete"));
      }
      if (typeof result.applyError === "string" && result.applyError.trim()) {
        setTelegramError(result.applyError.trim());
      }
  ...
  const upsertSlack = async () => {
    ...
    try {
      ...
      if (result.ok) {
        setSlackStatus(result.applied === false ? t("identities.saved_pending") : t("identities.saved"));
      } else {
        setSlackError(t("identities.failed_to_save"));
      }
      if (typeof result.applyError === "string" && result.applyError.trim()) {
  ...
  const deleteSlack = async (identityId: string) => {
    ...
    try {
      ...
      if (result.ok) {
        setSlackStatus(result.applied === false ? t("identities.deleted_pending") : t("identities.deleted"));
      } else {
        setSlackError(t("identities.failed_to_delete"));
      }
      if (typeof result.applyError === "string" && result.applyError.trim()) {
        setSlackError(result.applyError.trim());
      }
    ...
    <div class="space-y-6 max-w-[680px]">
      ...
      <div>
        ...
        <div class="mt-1.5 text-[11px] text-gray-8 font-mono break-all">
          {t("identities.workspace_scope")}: {scopedOpenworkBaseUrl().trim() || props.openworkServerUrl.trim() || t("identities.not_set")}
        </div>
      ...
      <Show when={!serverReady()}>
        <div class="rounded-xl border border-gray-4 bg-gray-1 p-5">
          <div class="text-sm font-semibold text-gray-12">{t("identities.connect_server")}</div>
          <div class="mt-1 text-xs text-gray-10">
            {t("identities.connect_server_hint")}
          </div>
        </div>
      </Show>
      ...
      <Show when={serverReady()}>
        ...
        <div class="rounded-xl border border-gray-4 bg-gray-1 p-4 space-y-3.5">
          ...
          <div class="flex gap-3">
            <StatusPill
              label={t("identities.channels")}
              value={t("identities.n_connected").replace("{n}", String(connectedChannelCount()))}
                ...
                <div class="border-t border-gray-4 px-4 py-4 space-y-3 animate-[fadeUp_0.2s_ease-out]">
                  ...
                  <Show when={telegramIdentities().length > 0}>
                          ...
                          <div class="flex items-center justify-between gap-3 rounded-lg border border-gray-4 bg-gray-1 px-3 py-2.5">
                            <div class="min-w-0">
                              ...
                              <div class="text-[11px] text-gray-9 mt-0.5 pl-3.5">
                                {item.enabled ? t("identities.enabled") : t("identities.disabled")} · {item.running ? t("identities.running") : t("identities.stopped")} · {item.access === "private" ? t("identities.private") : t("identities.public")}
                              </div>
                            </div>
                    ...
                    <div class="flex gap-2.5">
                      <div class="flex-1 rounded-lg border border-gray-4 bg-gray-2/50 px-3 py-2.5">
                        <div class="text-[11px] text-gray-9 mb-0.5">{t("identities.status")}</div>
                        <div class="flex items-center gap-1.5">
                          ...
                          <span class={`text-[13px] font-semibold ${
                            ...
                            {telegramIdentities().some((i) => i.running) ? t("identities.active") : t("identities.stopped")}
                          </span>
                        </div>
                      </div>
                      <div class="flex-1 rounded-lg border border-gray-4 bg-gray-2/50 px-3 py-2.5">
                        <div class="text-[11px] text-gray-9 mb-0.5">{t("identities.identities")}</div>
                        <div class="text-[13px] font-semibold text-gray-12">{t("identities.n_configured").replace("{n}", String(telegramIdentities().length))}</div>
                      </div>
                      <div class="flex-1 rounded-lg border border-gray-4 bg-gray-2/50 px-3 py-2.5">
                        <div class="text-[11px] text-gray-9 mb-0.5">{t("identities.channel")}</div>
                        <div class="text-[13px] font-semibold text-gray-12">
                          {health()?.channels.telegram ? t("identities.on") : t("identities.off")}
                        </div>
                      </div>
                    </div>
                  ...
                  <div class="space-y-2.5">
                      ...
                      <div class="rounded-xl border border-gray-4 bg-gray-2/60 px-3.5 py-3 space-y-2.5">
                        ...
                        <ol class="space-y-2 text-[12px] text-gray-10 leading-relaxed">
                          <li class="flex items-start gap-2">
                            ...
                            <span>
                              {t("identities.telegram_step1_prefix")} <a href="https://t.me/BotFather" target="_blank" rel="noreferrer" class="font-medium text-gray-12 underline">@BotFather</a> {t("identities.telegram_step1_suffix")} <code class="rounded bg-gray-3 px-1 py-0.5 font-mono text-[11px]">/newbot</code>.
                            </span>
                          </li>
                          <li class="flex items-start gap-2">
                          ...
                          <li class="flex items-start gap-2">
                            ...
                            <span>{t("identities.telegram_step3_prefix")} <span class="font-medium text-gray-12">{t("identities.public")}</span> {t("identities.telegram_step3_middle")} <span class="font-medium text-gray-12">{t("identities.private")}</span> {t("identities.telegram_step3_suffix")} <code class="rounded bg-gray-3 px-1 py-0.5 font-mono text-[11px]">/pair &lt;code&gt;</code>.</span>
                          </li>
                        </ol>
                      </div>
                    </Show>
                    ...
                    <div class="rounded-lg border border-gray-4 bg-gray-2/50 px-3 py-2 text-[11px] text-gray-10 leading-relaxed">
                      {t("identities.telegram_access_hint")}
                    </div>
                      ...
                      {(code) => (
                        <div class="rounded-xl border border-sky-7/25 bg-sky-1/40 px-3.5 py-3 space-y-2">
                          ...
                          <div class="text-[11px] text-sky-11/90 leading-relaxed">
                            {t("identities.pairing_instruction_prefix")} <code class="rounded bg-sky-3/60 px-1 py-0.5 font-mono text-[10px]">/pair {code()}</code>.
                          </div>
                          <div class="flex items-center gap-2">
                    ...
                    <Show when={telegramBotLink()}>
                      {(value) => (
                        ...
                        >
                          <Link size={14} />
                          {t("identities.open_bot_in_telegram").replace("{username}", telegramBotUsername() ?? "")}
                        </a>
                      )}
                    </Show>
                        ...
                        {(item) => (
                          <div class="flex items-center justify-between gap-3 rounded-lg border border-gray-4 bg-gray-1 px-3 py-2.5">
                            <div class="min-w-0">
                              ...
                              <div class="text-[11px] text-gray-9 mt-0.5 pl-3.5">
                                {item.enabled ? t("identities.enabled") : t("identities.disabled")} · {item.running ? t("identities.running") : t("identities.stopped")}
                              </div>
                            </div>
                            <div class="flex items-center gap-2 flex-shrink-0">
                              <Button
                      ...
                      <div class="flex-1 rounded-lg border border-gray-4 bg-gray-2/50 px-3 py-2.5">
                        ...
                        <div class="flex items-center gap-1.5">
                          ...
                          <span class={`text-[13px] font-semibold ${
                            ...
                            {slackIdentities().some((i) => i.running) ? t("identities.active") : t("identities.stopped")}
                          </span>
                        </div>
                      </div>
                      <div class="flex-1 rounded-lg border border-gray-4 bg-gray-2/50 px-3 py-2.5">
                        <div class="text-[11px] text-gray-9 mb-0.5">{t("identities.identities")}</div>
                        <div class="text-[13px] font-semibold text-gray-12">{t("identities.n_configured").replace("{n}", String(slackIdentities().length))}</div>
                      </div>
                      <div class="flex-1 rounded-lg border border-gray-4 bg-gray-2/50 px-3 py-2.5">
                        <div class="text-[11px] text-gray-9 mb-0.5">{t("identities.channel")}</div>
                        <div class="text-[13px] font-semibold text-gray-12">
                          {health()?.channels.slack ? t("identities.on") : t("identities.off")}
                        </div>
                      </div>
                    </div>
                  ...
                  <div class="space-y-2.5">
                    <Show when={slackIdentities().length === 0}>
                      <p class="text-[13px] text-gray-10 leading-relaxed">
                        {t("identities.slack_connect_hint")}
                      </p>
                    </Show>
        ...
        <div>
          ...
          <div class="text-xs text-gray-10 mt-2.5">
            {t("identities.routing_advanced_hint")}
          </div>
        </div>
          ...
          <div class="flex items-center justify-between gap-2">
            <div>
              ...
              <div class="text-[12px] text-gray-9 mt-0.5">
                {t("identities.agent_behavior_description")}
              </div>
            </div>
          ...
          <Show when={workspaceAgentStatus()}>
            {(value) => (
              <div class="rounded-lg border border-gray-4 bg-gray-2/40 px-3 py-2 text-[11px] text-gray-10">
                {t("identities.agent_scope_label")}: {value().loaded ? t("identities.loaded") : t("identities.missing")} · {t("identities.selected_agent")}: {value().selected || t("identities.none")}
              </div>
            )}
          </Show>

          <Show when={agentLoading()}>
            <div class="text-[11px] text-gray-9">{t("identities.loading_agent_file")}</div>
          </Show>
        ...
        <div class="rounded-xl border border-gray-4 bg-gray-1 p-4 space-y-3">
          <div>
            ...
            <div class="text-[12px] text-gray-9 mt-0.5">
              {t("identities.send_test_description")}
            </div>
          </div>

          <div class="grid gap-2 sm:grid-cols-2">
            <div>
              <label class="text-[12px] text-gray-9 block mb-1">{t("identities.channel")}</label>
              <select
            ...
            <div>
              <label class="text-[12px] text-gray-9 block mb-1">{t("identities.peer_id_optional")}</label>
              <input
          ...
          <div class="grid gap-2 sm:grid-cols-2">
            <div>
              <label class="text-[12px] text-gray-9 block mb-1">{t("identities.directory_optional")}</label>
              <input
                class="w-full rounded-lg border border-gray-4 bg-gray-1 px-3 py-2 text-sm text-gray-12 placeholder:text-gray-8"
