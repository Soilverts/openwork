...
export default function ConfigView(props: ConfigViewProps) {
  ...
  const openworkStatusLabel = createMemo(() => {
    switch (props.openworkServerStatus) {
      case "connected":
        return t("config.connected");
      case "limited":
        return t("config.limited");
      default:
        return t("config.not_connected");
    }
  });
  ...
  const reloadAvailabilityReason = createMemo(() => {
    if (!props.clientConnected) return t("config.connect_to_reload");
  ...
  const hostStatusLabel = createMemo(() => {
    if (!hostInfo()?.running) return t("config.offline");
    return t("config.available");
  });
  ...
  return (
    <section class="space-y-6">
      <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-2">
        <div class="text-sm font-medium text-gray-12">{t("config.workspace_config")}</div>
        <div class="text-xs text-gray-10">
          {t("config.workspace_config_description")}
        </div>
      ...
      <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-4">
        <div>
          <div class="text-sm font-medium text-gray-12">{t("config.engine_reload")}</div>
          <div class="text-xs text-gray-10">{t("config.engine_reload_description")}</div>
        </div>

        <div class="flex items-center justify-between bg-gray-1 p-3 rounded-xl border border-gray-6 gap-3">
          <div class="min-w-0 space-y-1">
            <div class="text-sm text-gray-12">{t("config.reload_now")}</div>
            <div class="text-xs text-gray-7">{t("config.reload_now_description")}</div>
            <Show when={props.anyActiveRuns}>
              <div class="text-[11px] text-amber-11">{t("config.reload_stop_warning")}</div>
            </Show>
        ...
        <div class="flex items-center justify-between bg-gray-1 p-3 rounded-xl border border-gray-6 gap-3">
          <div class="min-w-0 space-y-1">
            <div class="text-sm text-gray-12">{t("config.auto_reload")}</div>
            <div class="text-xs text-gray-7">{t("config.auto_reload_description")}</div>
          ...
          >
            {props.workspaceAutoReloadEnabled ? t("config.on") : t("config.off")}
          </Button>
        </div>

        <div class="flex items-center justify-between bg-gray-1 p-3 rounded-xl border border-gray-6 gap-3">
          <div class="min-w-0 space-y-1">
            <div class="text-sm text-gray-12">{t("config.resume_after_reload")}</div>
            <div class="text-xs text-gray-7">
              {t("config.resume_after_reload_description")}
            </div>
          </div>
          <Button
            variant="outline"
          ...
          >
            {props.workspaceAutoReloadResumeEnabled ? t("config.on") : t("config.off")}
          </Button>
        </div>
      </div>
      ...
      <Show when={props.developerMode}>
        <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-3">
          <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <div class="text-sm font-medium text-gray-12">{t("config.diagnostics_bundle")}</div>
              <div class="text-xs text-gray-10">{t("config.diagnostics_bundle_description")}</div>
            </div>
            <Button
              variant="secondary"
              class="text-xs h-8 py-0 px-3 shrink-0"
      ...
      <Show when={hostInfo()}>
        <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-4">
          <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <div class="text-sm font-medium text-gray-12">{t("config.server_sharing")}</div>
              <div class="text-xs text-gray-10">
                {t("config.server_sharing_description")}
              </div>
            </div>
          ...
          <div class="grid gap-3">
            ...
            <div class="flex items-center justify-between bg-gray-1 p-3 rounded-xl border border-gray-6 gap-3">
              <div class="min-w-0">
                <div class="text-xs font-medium text-gray-11">{t("config.access_token")}</div>
            ...
            <div class="flex items-center justify-between bg-gray-1 p-3 rounded-xl border border-gray-6 gap-3">
              <div class="min-w-0">
                <div class="text-xs font-medium text-gray-11">{t("config.server_token")}</div>
                <div class="text-xs text-gray-7 font-mono truncate">
                  {hostTokenVisible()
                    ? hostInfo()?.hostToken || "—"
                    : hostInfo()?.hostToken
      ...
      <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-4">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            <div class="text-sm font-medium text-gray-12">{t("config.openwork_server")}</div>
            <div class="text-xs text-gray-10">
              {t("config.openwork_server_description")}
            </div>
          </div>
        ...
        <div class="grid gap-3">
          <TextInput
            ...
            hint={t("config.server_url_input_hint")}
            disabled={props.busy}
          />

          <label class="block">
            <div class="mb-1 text-xs font-medium text-gray-11">{t("config.access_token")}</div>
            <div class="flex items-center gap-2">
              <input
                type={openworkTokenVisible() ? "text" : "password"}
                value={openworkToken()}
                onInput={(event) => setOpenworkToken(event.currentTarget.value)}
                placeholder={t("config.paste_token_placeholder")}
                disabled={props.busy}
            ...
            <div class="mt-1 text-xs text-gray-10">{t("config.token_optional_hint")}</div>
          </label>
        </div>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                const message = error instanceof Error ? error.message : t("config.connection_failed_short");
        ...
        <Show when={openworkStatusLabel() !== t("config.connected")}>
          <div class="text-xs text-gray-9">{t("config.server_connection_needed")}</div>
        </Show>
      </div>

      <div class="bg-gray-2/30 border border-gray-6/50 rounded-2xl p-5 space-y-2">
        <div class="text-sm font-medium text-gray-12">{t("config.messaging_identities")}</div>
        <div class="text-xs text-gray-10">
          {t("config.messaging_identities_description")}
        </div>
      </div>

      <Show when={!isTauriRuntime()}>
        <div class="text-xs text-gray-9">
          {t("config.desktop_features_hint")}
        </div>
      </Show>
    </section>
  );
